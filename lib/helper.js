...
var getKeys = function(third_party, query){
   ...
   if (undefined !== query.type && openiUtils.isTypeId(query.type)){
      if (undefined !== query.cloudlet && openiUtils.isCloudletId(query.cloudlet)){
         return { 'startKey' : [third_party, query.type, 0, query.cloudlet],
            'endKey' : [third_party, query.type, Number.MAX_VALUE, query.cloudlet + '\uefff' ]};
      }
...
var genGetMessage = function(msg, senderToDao, senderToClient, terminal_handler){
   ...
   var third_party = (msg.token["openi-token-type"] === "session") ? msg.token.cloudlet :  msg.token.context
